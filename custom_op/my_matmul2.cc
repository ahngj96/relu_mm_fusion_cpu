...
class TensorMatMul2Op : public OpKernel
{
    ...
    void Compute(OpKernelContext* ctx) override 
    {
		...
		//int K = input_tensor.dim_size(1) * 0.5;
		//int orig_K = input_tensor.dim_size(1) ;
		...
		for( int m = 0; m < M; m++ )
		{
			for( int n = 0; n < N; n++ )
			{
				float sum = 0.0;
                int k=0;
				for( k = 0; k < new_K; k+=bound )
				{
                    //for(int kk = 0 ; kk < bound*skip_rate ; kk++)
					    //sum = sum + a(m,k+kk) * b(k+kk,n);
					    sum = sum + a(m,k) * b(k,n);
					        sum = sum + a(m,k+1) * b(k+1,n);
				}
				for( k = new_K; k < K; k++ )
					    sum = sum + a(m,k) * b(k,n);
                ...
                if(sum <= 0) {
				    ...
				    for(k=2 ; k < new_K; k+=bound ){
                        //for(int kk = bound*skip_rate; kk < bound; kk++)
					        //sum = sum + a(m,k+kk) * b(k+kk,n);
					        sum = sum + a(m,k) * b(k,n);
                    }
				    out(m,n) = sum;
...
REGISTER_KERNEL_BUILDER(Name("TensorMatMul2").Device(DEVICE_CPU), TensorMatMul2Op);
